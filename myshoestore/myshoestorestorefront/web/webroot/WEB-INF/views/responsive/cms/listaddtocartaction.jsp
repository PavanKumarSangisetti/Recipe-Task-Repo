...
<c:if test="${not product.multidimensional }">
 ...
 <spring:theme code="basket.configure.product"/>
    ...
    <spring:url var="prodCompUrl" value=" {contextPath}/compare/product?=${product.code}" htmlEscape="false">
        <spring:param name="contextPath" value="${request.contextPath}" />
    </spring:url>

     <form id="prodCompForm" action="${fn:escapeXml(prodCompUrl)}" method="get">
    <button type="submit" name="prodCode" value="${fn:escapeXml(product.code)}" style = "margin: 20px; background-color: #FF8C00; font-size: 20px; color: white"> Compare your product with similar products</button>
     </form>
</c:if>
